...
def detect_faces_OpenCV_DNN(net, frame, min_conf=0.7):
...
# def Detect_faces_OpenFace(dnnModel,torchModel,frame):
#     blob = cv2.dnn.blobFromImage(
#         frame,                  # Image d'entrée (BGR ou RGB)
#         scalefactor=1.0,        # Facteur d'échelle (ex: 1/255.0)
#         size=(width, height),   # Taille cible (ex: (300, 300))
#         mean=(0, 0, 0),         # Valeurs moyennes à soustraire (B, G, R)
#         swapRB=False,           # Échange R et B (True si l'image est en RGB)
#         crop=False              # Recadrage après redimensionnement
#     )
#     return 

# net2 = cv2.dnn.readNetFromTorch("Codes/openface.nn4.small2.v1.t7")
...
if not video_capture.isOpened():
    print("camera couldn't be reached")
    exit()
while True:
